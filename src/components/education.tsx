            ...
            <div className="py-12 px-6 md:px-0 mx-auto md:w-4/5 relative">
...

                {
                    education.map((edu, index) => (
                        <div className='relative pb-5' key={index} >
                            <div className="absolute left-0 md:left-1/2 top-0 -translate-x-1/2 w-6 h-6 bg-slate-800 rounded-full border-4 border-white z-10"></div>
                            <div className="absolute left-0 md:left-1/2 -translate-x-1/2 w-1 h-full bg-zinc-700"></div>
                                ...
                                <div className={`relative flex items-center  ${index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`} >
                                    ...
                                    <div className={`w-full md:w-1/2 pb-3 ${index % 2 === 0 ? "md:pr-12" : "md:pl-12"} ml-5 md:ml-0`}>
                                        <motion.div whileHover={{ scale: 1.02, y: -5 }} transition={{ duration: 0.3 }}>
                                            <Card className="bg-slate-900 group hover:shadow-xl transition-all duration-300 border-none">
                                                <CardContent className="bg- text-white p-3 md:p-6 rounded-lg relative ">
                                                    <div className="flex items-start justify-between mb-4">
                                                        <div className="flex flex-col md:flex-row items-start gap-4">
                                                            ...
                                                            <div>
                                                                <h3 className="text-slate-300 text-lg md:text-xl font-bold mb-2 group-hover:text-primary transition-colors">
                                                                    {edu.degree}
                                                                </h3>
                                                                <p className=" text-slate-400 text-muted-foreground font-medium text-md md:text-lg">{edu.institution}</p>
                                                                <p className="text-slate-500 text-muted-foreground font-medium text-md md:text-lg">{edu.location}</p>
                                                            </div>
                                                        </div>
                                                    </div>
